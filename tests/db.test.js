...
describe('Testing out the basic database functions:', () => {
  ...
  describe('DB import function', () => {
    test('It should get a JSON file and pass it as an instance of a database', () => {
      expect(db.DB.getDB("package.json").name).toBe("package");
      expect(db.DB.getDB("package.json").path).toBe(path.resolve(process.cwd()));
      expect(db.DB.getDB("package.json").objects).toEqual(expect.anything());
    });
  });
});
